...
impl StakeNEARTokensAction {
    fn input_public_key(_context: &()) -> color_eyre::eyre::Result<near_crypto::PublicKey> {
        Ok(Input::new()
            .with_prompt("Enter a public key for this stake")
            .interact_text()?)
    }

    fn input_stake_amount(_context: &()) -> color_eyre::eyre::Result<crate::common::NearBalance> {
        Ok(Input::new()
